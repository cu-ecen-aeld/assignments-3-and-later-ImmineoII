...
Running ``echo "hello_world" > /dev/faulty`` cause the kernel error:
```
...
Data abort info:
  ...
  CM = 0, WnR = 1
user pgtable: 4k pages, 39-bit VAs, pgdp=00000000420f3000
...
sp : ffffffc008d1bd80
x29: ffffffc008d1bd80 x28: ffffff80020f8000 x27: 0000000000000000
x26: 0000000000000000 x25: 0000000000000000 x24: 0000000000000000
x23: 0000000040001000 x22: 000000000000000b x21: 0000005561bf21e0
x20: 0000005561bf21e0 x19: ffffff8002051300 x18: 0000000000000000
...
---[ end trace b04511a20f69835c ]---
```

The first line suggest us that kernel tried to deference a NULL pointer
>Unable to handle kernel NULL pointer dereference at virtual address 0000000000000000

Below the message print out the function where this faulty operation happen 
> faulty_write+0x14/0x20 [faulty]

So our hypothesis is that in faulty_write function inside the faulty kernel driver a NULL pointer deference happen.

## Module inspection

To inspect what happen inside our module we can, first of all read the code where in this case is obvious where is the problem, otherwise using objdump utility we can inspect our module operation launching ``./buildroot/output/host/bin/aarch64-linux-objdump -S ./buildroot/output/build/ldd-1325506db6f940835cc3a720794c95cd56a0e866/misc-modules/faulty.ko `` and looking at output:
```
./buildroot/output/build/ldd-1325506db6f940835cc3a720794c95cd56a0e866/misc-modules/faulty.ko:     file format elf64-littleaarch64
...
In the ``faulty_write`` function we can see how in the instruction at attress ``14`` trz to write at address stored in variable ``x1`` that point to address ``0``.